...
![image-20251222171853157](D:\Download\20251202_NoteBook\BookForAI\assets\image-20251222171853157.png)

+ **核心原理**：设计了Adapter结构，并将其嵌入Transformer的结构里面。针对每一个Transformer层，增加了两个Adapter结构(分别是多头注意力的投影之后和第二个FFN层之后)，在训练时，固定住原来预训练模型的参数不变，只对新增的 Adapter 结构和 Layer Norm 层进行微调，从而保证了训练的高效性。
+ <b style="color:green">Adapter的位置</b>
  + <b style="color:red">第一个Adapter</b>
    + **位置**：**Multi-head Attention** 层的输出之后，但在**Add**之前设置的
    + **计算流向**：`Multi-head Attention输出` $\rightarrow$ **`Adapter`** $\rightarrow$ `Add（残差连接）` $\rightarrow$ `Layer Norm（归一化）`
    + **关键点**：Adapter 产生的 “修正信息” 会被包含在残差加法中，然后整体进行归一化。
  + <b style="color:red">第二个Adapter</b>
    + **位置**： **Feed Forward** 层的输出之后，但在对应的**Add**之前设置的
    + **计算流向**：`Feed Forward 输出` $\rightarrow$ **`Adapter`** $\rightarrow$ `Add（残差连接）` $\rightarrow$ `Layer Norm（归一化）`
+ Adapter 的设计其实体现了 Adapter 的**补丁性质**
  + **微调本质**：它认为大模型原本的输出已经很好了，只是在特定任务上需要一点“微调”。
  + **残差逻辑**：由于 Adapter 后面跟着 `Add` 操作，它实际上学习的是相对于原始输出的**差值**
  + **冻结保护**：这种结构允许我们把 Attention 和 FFN 的巨大参数矩阵锁死，模型依然能通过这两个“小插件”来调整最终的输出向量。
+ **核心思想**：当作插件，不同任务就换不同的插件就能使用。



### <b style="color:blue">7. AdapterFusion</b>

![image-20251222195928639](D:\Download\20251202_NoteBook\BookForAI\assets\image-20251222195928639.png)

+ **问题产生原因**：为了让模型能够处理多种任务，传统的方法有两种：

  1. **顺序微调**：先让模型学会问题A，再去学习问题B。存在学完B忘记A的问题，或者由于学习顺序的不一致模型能力也变化。
  2. **多任务学习**：同时将不同任务的数据集让模型去学习。存在数据集中不同任务的数据有冲突，导致模型能力变差。

  由于Adapter Tuning的一个优势就是不用更新预训练模型的参数，因此衍生出将多个Adapater融合起来的想法。

+ **AdaoaterFusion执行过程**

  + **第一阶段**：【知识提取】在不同任务下引入各自的Adapter模块，用于学习特定任务的信息。
  + **第二阶段**：【知识组合】将PLM参数与特定于任务的Adapterca参数固定，引入新模块（AdapterFusion）来学习组合多个Adapter中的知识，以提高模型在目标任务中的表现。

+ **Transformer架构与AdapaterFusion架构对比**

  1. Transformer的一个标准的block
     + **第一阶段**：`输入` $\rightarrow$ `Multi-Head Attention` $\rightarrow$ **`Add & Norm`**
     + **第二阶段**：`第一阶段输出` $\rightarrow$ `FFN` $\rightarrow$ **`Add & Norm`**。
  2. AdapterFusion 是对Transformer的block中 “第二阶段” 之后作为**补充层**挂载的
     + **并行计算**：从第二个 `Add & Norm` 输出的结果，会**同时**进入多个并行的任务适配器
     + **Fusion 融合**：这些适配器的输出被送入 **Fusion 模块**（利用注意力机制分配权重并加权求和）。
     + **再次执行Add&Norm**

+ **Fusion模块**：本质上是一个**基于上下文的注意力机制**，它模仿了 Transformer 的 QKV 搜索过程，为每个 Adapter 分配一个权重。



### <b style="color:blue">8. AdapterDrop</b>

![image-20251222202241938](D:\Download\20251202_NoteBook\BookForAI\assets\image-20251222202241938.png)

+ 正常的Adapter架构是双Adapter架构<u>（Houlsby架构）</u>，即在多头注意力输出之后以及FFN输出之后都存在一个Adapter，但是在后来的研究发现在每个 Block 中放置两个 Adapter 有时是冗余的。
+ 研究证明，**只在 FFN（前馈网络）之后放置一个 Adapter** 就能达到与放置两个 Adapter 极其接近的性能。<u>（Pfeiffer架构）</u>

|     特性     | Houlsby 架构（双Adapter） | Pfeiffer 架构（单Adapter） |
| :----------: | :-----------------------: | :------------------------: |
| Adapter 数量 |     每一个block用2个      |      每一个block用1个      |
|   插入位置   |   Attention 后 + FFN 后   |        仅在 FFN 后         |
|   主要应用   |       早期经典 PEFT       | AdapterFusion, AdapterDrop |
|   设计偏好   |     追求最高表达能力      | **追求参数效率与速度平衡** |

+  使用Adapter进行微调
  + 训练阶段，参数量较全量微调大幅减小，因此训练时间变短。
  + 推理阶段，每一层都要多算一次 Adapter，因此推理时间变长。
+ **AdapterDrop** 通过在模型的某些层中**删除** Adapter 模块，来解决模型“推理速度慢”的问题。

+ **核心原理**
  + 模型的前几层（浅层）主要学习通用的语法特征，对特定任务的适配需求较低。因此可以**直接删掉浅层的 Adapter**。
  + 通过减少需要计算的层数，在不显著损失性能的前提下，大幅**缩短推理时间**并提高吞吐量。
  + 实验证明，即使丢弃了一半以上的浅层 Adapter，模型的准确率下降通常非常微小。
+ **AdapterDrop主要操作**
  + <b style="color:green">训练阶段</b>：训练时，随机丢弃每一层中的 Adapter。这类似于 Dropout，强迫模型剩下的 Adapter 变得更加“鲁棒”，学会即使在缺少浅层适配信息的情况下也能完成任务。
  + <b style="color:green">推理阶段</b>：在实际使用时，根据对速度的要求，直接砍掉前 $K$ 层的 Adapter。

>1. **Adapter**：通过插入小模块实现高效微调，但推理慢。
>2. **AdapterFusion**：通过 Attention 机制融合多个 Adapter 处理多任务。
>3. **AdapterDrop**：通过删掉不重要层（通常是浅层）的 Adapter 来找回推理速度。





### <b style="color:blue">9. LoRA</b>

![image-20251222220118922](D:\Download\20251202_NoteBook\BookForAI\assets\image-20251222220118922.png)

+ **核心思想**：低秩分解来模拟参数的改变量，从而以极小的参数量来实现大模型的间接训练。
+ **执行过程**
  + **原始权重 $W$**：型中原有的全连接层权重（如 $W_q, W_v$），在微调期间被**冻结（Frozen）**，不接收任何梯度更新。
  + **旁路通路 $\Delta W$**：在 $W$ 旁边增加一个并行的支路。为了节省参数，这个支路被分解为两个小矩阵 $A$ 和 $B$ 的乘积：$\Delta W = B \cdot A$
    + **矩阵 $A$**：维度为 $d \times r$，负责**降维**。它通常使用高斯分布随机初始化。
    + **矩阵 $B$**：维度为 $r \times d$，负责**升维**。它初始化为全 0，以确保在训练开始时，旁路输出为 0，不影响模型原有性能。
    + **秩 $r$**：这是一个超参数，通常设置得非常小（如 4、8 或 16），远小于原始维度 $d$。
+ **数据流向**：在训练过程中，输入向量 $x$ 会同时流经两条路径。
  1. **主路径**：通过冻结的 $W$，得到原始特征 $xW$。
  2. **旁路**：依次通过 $A$ 和 $B$，得到微调增量 $xBA$。
  3. **求和**：将两者的结果相加得到最终输出：$h = xW + xBA \cdot \frac{\alpha}{r}$。
     + 注：$\alpha$ 是一个缩放因子，用于控制微调部分对模型的影响力。



### <b style="color:blue">10. AdaLoRA</b>

+ **LoRA存在问题**
  1. **预先指定本征秩 $r$ 相同**：在标准 LoRA 中，开发者通常会全局设置一个固定的秩，并应用到模型所有的层和所有矩阵。这种做法忽略了模型内部的**权重重要性差异**，不同模块的$W$的贡献不一样。
     + 参数浪费（在不重要的层次r多余）
     + 表达瓶颈（在关键层r不足）
  2. **只训练了 Attention，没有训练 FFN**：在 LoRA 的原始论文中实验主要集中在 Transformer 的 **Attention模块上（即 $W_q, W_k, W_v, W_o$）**
     + 前馈神经网络（FFN）占据了 Transformer 约 **2/3** 的参数量。它就像是模型的“知识库”，存储了大量的具体事实信息。但是却没有对FFN进行训练。
     + 许多研究发现，微调 FFN 对模型吸收新领域知识（领域自适应）至关重要。
     + 如果只做 Attention 的 LoRA，模型可能只是学会了新的“对话格式”，而没有真正更新其底层的知识内容。
+ **基于存在问题AdaLoRA目的**
  - 不能预先指定矩阵的秩，需要动态更新增量矩阵的R，因为权重矩阵的重要性在不同模块和层之间存在显著差异。
  - 需要找到更加重要的矩阵，分配更多的参数，裁剪不重要的矩阵。找到重要的矩阵，可以提升模型效果；而裁剪不重要的矩阵，可以降低参数计算量，降低模型效果差的风险。
  - AdaLoRA根据权重矩阵的重要性得分，在权重矩阵之间自适应地分配参数预算。

+ <b style="color:red">数学实现</b>

  + AdaLoRA 不再直接学习矩阵 $A$ 和 $B$，而是将权重增量 $\Delta W$ 参数化为**奇异值分解 (SVD)** 的形式
    $$
    \Delta W = P \cdot \Lambda \cdot Q
    $$

    + **$P$ 和 $Q$**：左奇异向量矩阵和右奇异向量矩阵。
    + **$\Lambda$**：一个对角矩阵，其对角线上的值即为**奇异值**。
    + 奇异值的大小直接反映了该维度的重要性。AdaLoRA 通过操纵 $\Lambda$ 来控制秩的大小。如果某个奇异值降为零，实际上就降低了该层的秩 $r$

+ **解决问题**

  + 解决“秩 $r$ 相同”的问题：允许模型在训练过程中自动调整每一层的 $r$，例如：在 LLaMA 中，它可能会自动发现第 28 层的 FFN 非常关键（分配 $r=16$），而第 2 层的 Attention 几乎没用（分配 $r=2$）。
  + 解决“忽略 FFN”的问题：AdaLoRA 通常将 **Attention 和 FFN 所有的线性层**都纳入考虑范围。



### <b style="color:blue">11. QLoRA</b>

![image-20251223000702761](D:\Download\20251202_NoteBook\BookForAI\assets\image-20251223000702761.png)

+ **主要创新点**
  + <b style="color:green">NF4 数据格式</b>：
    + 一种专门为神经网络权重设计的信息理论上最优的数据类型
    + 由于预训练模型的权重通常呈正态分布，NF4 通过数学变换将权重映射到 4-bit 空间，其精度显著优于标准的 4-bit 整数或浮点数。
    + 将原始高精度（如 FP16/BF16）基座模型以 **4-bit NF4** 格式加载到显存中。单个数据从16位存储变成4位存储。
    + 将原始权重 $W$ 压缩为 4-bit（冻结状态）
  + <b style="color:green">计算时的实时反量化</b>
    + 在 4-bit 基座之上，使用 16-bit（BF16/FP16）的精度训练 LoRA 旁路。
    + 在进行前向或反向传播时，QLoRA 会将 4-bit 权重临时“反量化”回 **BF16** 精度，用于与输入的隐藏层状态进行计算。
    + 梯度仅在 LoRA 的旁路（通常保持 BF16 高精度）中流动并更新，原始 4-bit 基座保持不变。
  + <b style="color:green">分页优化器</b>
    + 利用了 NVIDIA 显卡的统一内存管理功能。
    + 当显存即将溢出（OOM）时，它会将优化器状态自动转移到 CPU 内存（RAM）中，待需要计算时再换回，有效防止了训练崩溃。
  + <b style="color:green">双量化</b>
    + **普通量化**：$权重 \xrightarrow{量化} 4\text{-bit} + 量化常数(32\text{-bit})$
    + **双量化**：$权重 \xrightarrow{量化} 4\text{-bit} + (量化常数 \xrightarrow{再量化} 8\text{-bit})$
+ **硬件流转逻辑**
  + **存储态：极致节约的 4-bit (NF4)**
    + **状态**：预训练模型（PLM）的权重 $W$ 以 **4-bit NF4** 格式静静地躺在显存里。
    + **优势**：原本需要 16GB 的模型，现在只需要约 4GB 左右。这部分数据在整个训练过程中确实是被**锁定（Frozen）**的，完全不参与更新。
  + **计算态：瞬间恢复的 BF16**
    + 输入数据进行前向传播（Forward Pass）时，发生了以下动作。
    + **即时转换**：计算内核（CUDA Kernel）从显存抓取 4-bit 数据时，会瞬间根据量化常数将其“解压”还原为 **BF16/FP16**
    + **计算叠加**：
      + **主路计算**：用还原出的 BF16 权重与输入 $x$ 做矩阵乘法。
      + **旁路计算**：输入 $x$ 同时流经一直保持 **BF16** 格式的 LoRA 旁路。
      + **求和**：将两个 BF16 结果相加，得到最终的激活值。
  + **更新态：精准修正的 BF16 梯度**
    + **反向传播**：误差信号（梯度）传回来时，它会跳过 4-bit 的基座，只进入 **BF16 格式的 LoRA 旁路**。
    + **权重修改**：优化器根据梯度，只修改旁路里的那两个小矩阵。
    + **内存平衡**：因为大部分显存被 4-bit 基座占用了，而 32-bit 的优化器状态只针对极少量的 LoRA 参数，所以显存压力极小。
